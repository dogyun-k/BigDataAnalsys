...
def remove_overlap(file_name):
    ...
    with open(file_name+'_removed_overlap.txt', 'w', encoding='UTF-8') as f:
...
def normalization(address):
    ...
    # 검색에 방해되는 단어 지우기
    address = re.sub('[부근, 건너, 횡단보도, 모퉁이, 소화전, 버스정류장, 버스정류]', '', address)
...
def refactoring_file(file_name):
    ...
    new_f = open(file_name+'_r.csv', 'w', newline='', encoding='utf-8-sig')
...
# remove_overlap('2021_r')
# refactoring_file('2021')
# check_ok('2021')

print(get_geocode("대구 북구 읍내동 칠곡중앙대로 125"))